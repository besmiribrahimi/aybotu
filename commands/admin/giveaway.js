  ...
  async execute(interaction) {
    ...
    if (mode === 'exclusive' && !requiredRole) {
      return interaction.reply({ 
        content: '˚ ༘ ⋆｡˚ Please specify a required role for exclusive giveaways! ♡', 
        ephemeral: true 
      });
    }
    ...
    const embedColor = isExclusive ? '#E8A2D0' : '#FFD1DC';
    const modeTitle = isExclusive ? '♡   .   —  ꒰ ✨ EXCLUSIVE GIVEAWAY ✨ ꒱' : '♡   .   —  ꒰ 🌸 GIVEAWAY 🌸 ꒱';
    const modeEmoji = isExclusive ? '💎' : '🎀';
    const footerText = isExclusive ? '୭ ˚. ᵎᵎ ✨ °  Special members only~!' : '୭ ˚. ᵎᵎ 🍥 °  Good luck everyone!';

    let description = `୨୧ ⏔⏔⏔⏔♡⏔⏔⏔⏔ ୨୧\n\n`;
    description += `　　　　　　𓂃 ִֶָ𐀔　${modeEmoji} Prize ${modeEmoji}　𓂃 ִֶָ𐀔\n\n`;
    description += `**${prize}**\n\n`;
    
    if (isExclusive) {
      description += `🔒 **Exclusive to:** ${requiredRole}\n`;
      description += `˚₊‧꒰ა Only special members can enter! ꒱჻ ⊹\n\n`;
    }
    
    description += `⊹ ࣪✧ Click the button to enter! ˖ °\n`;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({ name: '⊹ ࣪✧ Hosted by ˖ °', value: interaction.user.toString(), inline: true })
      .setFooter({ text: footerText })
      .setTimestamp(new Date(endTime));

    // Softer, kawaii button styling - using Secondary (gray) for soft look
    ...
    try {
      ...
      await interaction.reply({ 
        content: `⊹ ࣪✧ ${modeText} giveaway started in ${channel}! ♡`, 
        ephemeral: true 
      });
    } catch (error) {
      await interaction.reply({ 
